        ...
        echo $args['before_widget'];
			if ($widget_posts->have_posts()) :
				...
				if ($instance['category_label'] == 0) {
					$caption_class = ' mh-posts-grid-hide-caption';
				} else {
					$caption_class = '';
				}
				while ($widget_posts->have_posts()) : $widget_posts->the_post();
					if ($counter == 1) {
						echo '<div class="mh-row clearfix mh-posts-grid-widget' . esc_attr($caption_class) . '">' . "\n";
					}
					if ($counter >= 1) {
					...
					if ($counter % 3 == 0 && $counter != $instance['postcount']) {
						echo '</div>' . "\n";
						echo '<div class="mh-row clearfix mh-posts-grid-widget mh-posts-grid-widget-more' . esc_attr($caption_class) . '">' . "\n";
					}
					$counter++;
				endwhile;
				echo '</div>' . "\n";
