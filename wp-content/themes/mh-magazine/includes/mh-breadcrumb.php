					...
					if (is_single() && get_post_type() == 'post' && !is_attachment()) {
						...
						if ($post->post_parent) {
							...
							foreach ($parents as $parent_id) {
								echo $before_link . '<a href="' . esc_url(get_permalink($parent_id)) . '" itemprop="url">' . $before_title . get_the_title($parent_id) . $close_span . '</a>' . $close_span . $delimiter;
							}
						}
						echo get_the_title();
						...
						echo esc_html__('Search', 'mh-magazine') . $delimiter . get_search_query();
					} elseif (is_date()) {
